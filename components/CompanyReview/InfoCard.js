...
// // components/CompanyReview/InfoCard.js
// import React from "react";
// import StarRating from "../StarRating/StarRating";

// const InfoCard = ({ data, averageRating, totalReviews }) => { // Accept averageRating and totalReviews as props
//     if (!data) return null;

//     const { acf ,title} = data;

//     //const rating = parseFloat(acf?.rating) || 0; // No longer using acf.rating
//     //const reviews = parseFloat(acf?.user_reviews) || 0; // No longer using acf.user_reviews
//     const pricing = acf?.pricing_plans?.split("\n") || [];
//     const splitPricingLine = (line) => {
//         const match = line.match(/([\D\s]+?)(\$\s*[\d\.]+[\s-]*[\$\d\.]*)/);
//         if (match) {
...
//                 <p className="text-gray-600 text-center">
//                     Based on expert ratings and {totalReviews} user reviews
//                 </p>
//             </div>
//             <div className="w-full flex flex-col items-start justify-start flex-1 gap-6 md:p-4 p-0 lg:border border-0 md:order-2 order-3 border-gray-300 rounded-xl md:flex-row">
//                 <div className="flex-1 w-full">
...
const InfoCard = ({ data, averageRating, totalReviews }) => {
    if (!data) return null;
    ...
    return (
        <div className="flex flex-col justify-between gap-4 lg:flex-row bg-white md:p-10 p-5 border border-gray-300 shadow-md rounded-2xl -mt-28">
            {averageRating !== null ? ( // Conditionally render based on averageRating
                <div className="w-full flex flex-col items-center justify-center md:p-4 p-0 lg:border border-0 border-gray-300 rounded-xl lg:w-1/4 space-y-2">
                    <span className="text-4xl font-bold text-gray-800">
                        {averageRating.toFixed(1)}
                    </span>
                    <StarRating rating={averageRating} size="text-base" />
