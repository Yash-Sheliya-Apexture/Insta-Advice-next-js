...
const SearchBar = ({ specificIds }) => {
  ...
  const handleCloseSearch = () => {
    setIsSearchVisible(false);
    document.body.classList.remove("overflow-hidden");
    setSearchQuery(""); // Reset the search query when closing the box
    setSearchResults(initialResults); // Also reset the search results to initial ones
  };
  ...
  const fetchRatingsAndReviews = useCallback(
    async (postId) => {
      if (!jwt) {
        return { averageRating: 0, totalReviews: 0 };
      }
        ...
        const postsWithRatings = await Promise.all(
          data.map(async (item) => {
            const { averageRating, totalReviews } =
              await fetchRatingsAndReviews(item.id);
            return {
              ...item,
              averageRating: averageRating,
              totalReviews: totalReviews,
  ...
  useEffect(() => {
    const getInitialResults = async () => {
      setIsLoading(true);
      try {
        const formattedData = await fetchSearchResults("", specificIds);
        setSearchResults(formattedData);
        setInitialResults(formattedData);
      } catch (error) {
  ...
  const handleSearchChange = async (e) => {
    ...
    if (!query) {
      ...
      // Fetch all data (without initial filtering)
  ...
  const renderSearchResults = () => {
    if (isLoading) {
      return (
        <div className="py-16 text-center text-light-color"><LoadingSpinner/></div>
      );
    }

    if (searchResults.length === 0) {
      return (
        <div className="py-16 text-center text-light-color">
          No results found
        </div>
      );
    }
    ...
    return (
      <div className="mt-6 flex flex-col gap-6">
        {resultsToShow.map((result) => (
          <div
            key={result.id}
            className="p-6 border border-gray-300 rounded-xl shadow-lg flex sm:flex-row flex-col justify-between items-center bg-white"
          >
            <Image
              src={result.image}
              alt={result.title}
              width={200}
              height={45}
              className="w-36 h-12 object-contain rounded-lg"
            />
            <div>
              <div className="flex flex-col sm:flex-row items-center gap-3">
                <StarRating
                  rating={parseFloat(result.averageRating)}
                  size="text-lg"
                />
                <div className="flex space-x-2">
  ...
  return (
    <div>
      <button
        ...
        className="flex items-center px-2 text-dark-color cursor-pointer"
      >
        <FaSearch className="text-gray-700" />
      </button>
      ...
      {isSearchVisible && (
        <div className="fixed inset-0 bg-black/60 z-50 flex justify-center items-start py-0 lg:py-4">
          <div
            className="relative w-full h-full lg:max-w-3xl p-4 bg-white lg:rounded-3xl rounded-none overflow-x-auto"
            ref={searchRef}
          >
            <div className="flex relative">
              <input
              ...
              <button
                ...
                className="absolute cursor-pointer inset-y-0 right-2 flex items-center px-2 text-dark-color"
              >
                <FaTimes />
              </button>
            </div>
